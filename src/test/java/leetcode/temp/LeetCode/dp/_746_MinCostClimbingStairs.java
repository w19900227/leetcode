...
//        int[] list = new int[]{841, 462};
    ...
    public int minCostClimbingStairs(int[] cost) {
        ...
        for (int i = cost.length - 1; 0 <= i; i--) {
            int last1 = (i + 1 < cost.length) ? result[i + 1] : 0;
            int last2 = (i + 2 < cost.length) ? result[i + 2] : 0;
            result[i] = Math.min(last1, last2) + cost[i];
        }
...
//    }
//
//    private int helper(int[] cost, int index, Integer[] sum) {
//        if (cost.length - 2 <= index) return cost[index];
//        if (sum[index] != null) return sum[index];
//        int min = Math.min(helper(cost, index + 1, sum), helper(cost, index + 2, sum)) + cost[index];
//        sum[index] = min;
//        return min;
//    }
}